$(document).ready(function() {
	G.addControl("PublicRankingPage", Control.sub({
		ref: 'htmlContainer',
		...
		initialize: function() {
			...
			this.inDocument(function() {
				$("#dealershipNameHole").append(self.pageData().dealershipName);
				$('#viewDealershipDetailsButton').attr({ href: '/store/' + self.pageData().dealershipLocationCode })
				$('#listingsButton').attr({ href: '/publicranking/' + self.pageData().dealershipLocationCode })
				$("#address_hole").append(
					$('<div>').append(
						self.pageData().address.street
					),
					$('<div>').append(
						self.pageData().address.region
					)
				);				
				...
				bind = function() {
					$.map($('.selectparent li a'), function(elem) {
						$(elem).click(function() {
							...
							document.location = '/publicranking/' + self.pageData().dealershipLocationCode + url;							
						});
					});	
				}
